...
<template>
  <div class="relative location-selector-container font-khmer">
    ...
    <!-- 1. INLINE ACCORDION (USED IN MOBILE DRAWER) -->
    <div
      v-if="isOpen && inline"
      class="mt-2 p-2.5 bg-slate-50 dark:bg-slate-800/90 rounded-2xl border border-slate-200/90 dark:border-slate-700 shadow-2xs space-y-2 animate-fadeIn"
    >
      <!-- Search Input -->
      <div class="relative">
        ...
        <input
          ...
          class="w-full pl-8 pr-3 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-[#0D47A1]"
        />
      </div>
      ...
      <button
        ...
        :class="[
          ...
          currentProvinceId === 'all'
            ? 'bg-blue-50 dark:bg-blue-950/60 text-[#0D47A1] dark:text-blue-300 font-black shadow-xs ring-1 ring-blue-500/20'
            : 'bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 hover:bg-slate-100/80 dark:hover:bg-slate-800 border border-slate-200/60 dark:border-slate-700/60'
        ]"
        type="button"
      >
        <div class="flex items-center gap-2 min-w-0">
      ...
      <div class="max-h-56 overflow-y-auto space-y-1 pr-1 custom-scrollbar">
        ...
        <div v-if="filteredProvinces.length === 0" class="py-3 text-center text-xs text-slate-400">
