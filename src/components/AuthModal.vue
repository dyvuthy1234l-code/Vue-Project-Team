  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      login({
        ...
        phone: !input.includes('@') ? input : (isAdmin ? '023 888 999' : '012 345 678'),
...
// Quick Demo Login as Admin - Opens Admin Dashboard directly
function loginDemoAdmin() {
  loginForm.emailOrPhone = 'admin@camlife.gov.kh'
  loginForm.password = 'admin2026'
  errorMessage.value = ''
  successMessage.value = currentLanguage.value === 'kh'
    ? 'ចូលជាអ្នកគ្រប់គ្រង (Admin Officer) ជោគជ័យ! កំពុងបើកផ្ទាំង Admin...'
    : 'Signed in as Admin Officer! Opening Admin Dashboard...'
