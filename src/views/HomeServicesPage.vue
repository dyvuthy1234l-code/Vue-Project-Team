...
<template>
  <div class="w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-5 sm:py-10 space-y-5 sm:space-y-8 font-khmer overflow-x-clip min-w-0">
    ...
    <div class="rounded-2xl sm:rounded-3xl bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 text-white p-4 sm:p-7 shadow-lg relative overflow-hidden">
      <div class="relative z-10 space-y-3.5 sm:space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          <div class="flex items-center gap-2.5 sm:gap-3">
            <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center text-white ring-1 ring-white/30 shrink-0">
              <ShieldCheck class="w-5 h-5 sm:w-6 sm:h-6 text-emerald-300" />
            </div>
            <div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 pt-1 text-xs font-bold text-white">
          <div class="flex items-center gap-2 p-2 sm:p-3 rounded-xl sm:rounded-2xl bg-white/10 backdrop-blur-xs border border-white/10 min-w-0">
            <Award class="w-4 h-4 sm:w-5 sm:h-5 text-amber-300 shrink-0" />
            <div class="min-w-0">
              <p class="text-[11px] sm:text-xs font-black truncate">{{ currentLanguage === 'kh' ? 'ផ្ទៀងផ្ទាត់អត្តសញ្ញាណ' : '100% Verified ID' }}</p>
              <p class="text-[9px] sm:text-[10px] text-blue-200 font-normal truncate">{{ currentLanguage === 'kh' ? 'មានប្រវត្តិរូបត្រឹមត្រូវ' : 'Background checked' }}</p>
            </div>
          </div>
          <div class="flex items-center gap-2 p-2 sm:p-3 rounded-xl sm:rounded-2xl bg-white/10 backdrop-blur-xs border border-white/10 min-w-0">
            <Clock class="w-4 h-4 sm:w-5 sm:h-5 text-teal-300 shrink-0" />
            <div class="min-w-0">
              <p class="text-[11px] sm:text-xs font-black truncate">{{ currentLanguage === 'kh' ? 'មកដល់ក្នុង 30 នាទី' : 'Fast Response' }}</p>
              <p class="text-[9px] sm:text-[10px] text-blue-200 font-normal truncate">{{ currentLanguage === 'kh' ? 'សេវាបន្ទាន់រហ័ស' : 'Same-day available' }}</p>
            </div>
          </div>
          <div class="flex items-center gap-2 p-2 sm:p-3 rounded-xl sm:rounded-2xl bg-white/10 backdrop-blur-xs border border-white/10 min-w-0">
            <DollarSign class="w-4 h-4 sm:w-5 sm:h-5 text-emerald-300 shrink-0" />
            <div class="min-w-0">
              <p class="text-[11px] sm:text-xs font-black truncate">{{ currentLanguage === 'kh' ? 'តម្លៃប៉ាន់ស្មានថេរ' : 'Clear Pricing' }}</p>
              <p class="text-[9px] sm:text-[10px] text-blue-200 font-normal truncate">{{ currentLanguage === 'kh' ? 'គ្មានការទារលើស' : 'No hidden fees' }}</p>
            </div>
          </div>
          <div class="flex items-center gap-2 p-2 sm:p-3 rounded-xl sm:rounded-2xl bg-white/10 backdrop-blur-xs border border-white/10 min-w-0">
            <CheckCircle2 class="w-4 h-4 sm:w-5 sm:h-5 text-sky-300 shrink-0" />
            <div class="min-w-0">
              <p class="text-[11px] sm:text-xs font-black truncate">{{ currentLanguage === 'kh' ? 'ធានាគុណភាព 30 ថ្ងៃ' : '30-Day Guarantee' }}</p>
              <p class="text-[9px] sm:text-[10px] text-blue-200 font-normal truncate">{{ currentLanguage === 'kh' ? 'ជួសជុលឡើងវិញបើខូច' : 'Workmanship covered' }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl sm:rounded-3xl border border-slate-200/90 dark:border-slate-700 p-3.5 sm:p-6 shadow-sm space-y-3.5 sm:space-y-5">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-12 gap-2.5 sm:gap-3">
        ...
        <div class="sm:col-span-1 md:col-span-3">
          <select
            ...
            class="w-full px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl sm:rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-xs sm:text-sm font-bold text-[#0A2540] dark:text-white focus:outline-hidden focus:ring-2 focus:ring-[#0D47A1]"
          >
      ...
      <!-- Quick Issue Chips (Horizontal Scroll on Mobile) -->
      <div class="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-0.5 -mx-1 px-1 text-xs">
        <span class="text-slate-400 font-bold shrink-0 text-[11px] sm:text-xs">{{ currentLanguage === 'kh' ? 'បញ្ហាជួបញឹកញាប់៖' : 'Common Issues:' }}</span>
        <button
          v-for="prob in urgentProblems"
          :key="prob.id"
          @click="applyProblem(prob.query)"
          type="button"
          class="px-2.5 sm:px-3 py-1 rounded-xl bg-slate-100 dark:bg-slate-700/80 hover:bg-blue-50 dark:hover:bg-blue-900/40 text-slate-600 dark:text-slate-300 hover:text-[#0D47A1] dark:hover:text-blue-300 font-bold border border-slate-200 dark:border-slate-600 transition-all cursor-pointer shrink-0 text-[11px] sm:text-xs whitespace-nowrap"
        >
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
      <p class="text-xs font-bold text-slate-500 dark:text-slate-400">
        {{ currentLanguage === 'kh' ? 'រកឃើញ' : 'Found' }}
        <span class="text-[#0A2540] dark:text-white font-black">{{ filteredServices.length }}</span>
        {{ currentLanguage === 'kh' ? 'សេវាកម្មជាងជំនាញបានផ្ទៀងផ្ទាត់' : 'verified home specialists' }}
      </p>
      ...
      >
        <div class="min-w-0">
          ...
          <div
            ...
            class="relative h-28 sm:h-44 w-full overflow-hidden bg-slate-100 dark:bg-slate-700 cursor-pointer"
          >
            <LazyImage
              v-if="service.image"
              :src="service.image"
            ...
            >
              <Wrench class="w-8 h-8 sm:w-12 sm:h-12 opacity-50" />
            </div>
            ...
            <div class="absolute top-2 sm:top-3 left-2 sm:left-3 right-2 sm:right-3 flex items-center justify-between gap-1 pointer-events-none">
              <span class="px-1.5 sm:px-2.5 py-0.5 sm:py-1 text-[9px] sm:text-[11px] font-black uppercase tracking-wider rounded-lg sm:rounded-xl bg-white/95 dark:bg-slate-900/90 text-blue-800 dark:text-blue-300 border border-blue-200/60 shadow-2xs backdrop-blur-md flex items-center gap-1 max-w-[65%] truncate">
                <Wrench class="w-2.5 h-2.5 sm:w-3 sm:h-3 text-blue-600 shrink-0" />
                <span class="truncate">{{ service.category }}</span>
              </span>

              <span class="inline-flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-full text-[9px] sm:text-[10px] font-extrabold bg-emerald-600 text-white shadow-2xs shrink-0">
                <CheckCircle2 class="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5" />
                <span class="hidden sm:inline">Available Today</span>
                <span class="sm:hidden">ទំនេរ</span>
              </span>
            </div>

            <!-- Price Estimate Floating Badge -->
            <div class="absolute bottom-2 sm:bottom-3 left-2 sm:left-3 right-2 sm:right-3 flex items-center justify-between gap-1 text-white">
              <span class="inline-flex items-center gap-0.5 sm:gap-1 text-[9px] sm:text-xs font-black bg-emerald-600/95 backdrop-blur-md px-1.5 sm:px-3 py-0.5 sm:py-1 rounded-md sm:rounded-xl border border-emerald-400/40 shadow-2xs shrink-0">
                <DollarSign class="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5" />
                <span>{{ service.priceEstimate || '$15 - $35' }}</span>
              </span>
              <span class="inline-flex items-center gap-0.5 sm:gap-1 text-[9px] sm:text-[11px] font-bold bg-slate-900/85 backdrop-blur-md px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-md sm:rounded-xl shadow-2xs max-w-[50%] truncate">
                <MapPin class="w-2.5 h-2.5 sm:w-3 sm:h-3 text-red-400 shrink-0" />
          ...
          <div class="p-2.5 sm:p-5 space-y-2 sm:space-y-3.5 min-w-0">
            ...
            <div class="min-w-0">
              <div class="flex items-center gap-1 text-[10px] sm:text-xs text-slate-400 font-bold mb-0.5 truncate">
                <span class="truncate">{{ service.provider }}</span>
                <ShieldCheck class="w-3 h-3 sm:w-3.5 sm:h-3.5 text-blue-600 dark:text-blue-400 shrink-0" />
              </div>
              <h3
                @click="openServiceDetail(service)"
                class="text-xs sm:text-base font-black text-[#0A2540] dark:text-white group-hover:text-[#0D47A1] dark:group-hover:text-blue-400 transition-colors leading-snug line-clamp-2 cursor-pointer"
              >
            ...
            <!-- Rating & Reviews Pill -->
            <div class="flex items-center gap-1.5 sm:gap-3 text-[10px] sm:text-xs flex-wrap">
              <div class="flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-lg sm:rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200/60 dark:border-amber-900/40 text-amber-700 dark:text-amber-300 font-black shrink-0">
                <Star class="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 fill-amber-400 text-amber-400" />
                <span>{{ service.rating.toFixed(1) }}</span>
              </div>
              <span class="text-slate-400 font-medium truncate text-[9px] sm:text-xs">
                {{ service.reviews || 25 }} {{ currentLanguage === 'kh' ? 'ការងារ' : 'jobs' }}
              </span>
            </div>

            <!-- Description -->
            <p class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 leading-relaxed line-clamp-1 sm:line-clamp-2">
              {{ localized(service.description, service.descriptionKh) }}
            </p>

            <!-- Trust Highlights -->
            <div class="pt-1.5 sm:pt-2 border-t border-slate-100 dark:border-slate-700/60 flex items-center justify-between text-[9px] sm:text-[11px] font-bold text-slate-500 dark:text-slate-400">
              <span class="flex items-center gap-0.5 sm:gap-1 text-emerald-600 dark:text-emerald-400 truncate">
                <CheckCircle2 class="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 shrink-0" />
                <span class="truncate">{{ currentLanguage === 'kh' ? 'ធានា 30ថ្ងៃ' : '30-Day' }}</span>
              </span>
              <span class="flex items-center gap-0.5 sm:gap-1 text-[#0D47A1] dark:text-blue-400 truncate">
                <Clock class="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 shrink-0" />
                <span class="truncate">{{ currentLanguage === 'kh' ? 'រហ័ស' : 'Fast' }}</span>
              </span>
            </div>
          </div>
        </div>
        ...
        <div class="p-2.5 sm:p-5 pt-0">
          <div class="pt-2 sm:pt-3 border-t border-slate-100 dark:border-slate-700/60 flex items-center gap-1 sm:gap-2">
            <!-- View Detail Button -->
            <button
              type="button"
              @click="openServiceDetail(service)"
              class="flex-1 py-1.5 sm:py-2.5 px-1 sm:px-2.5 rounded-xl sm:rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/60 hover:bg-blue-50 hover:border-blue-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 hover:text-[#0D47A1] dark:hover:text-blue-400 font-black text-[10px] sm:text-xs flex items-center justify-center gap-0.5 sm:gap-1.5 transition-all cursor-pointer shadow-2xs"
            >
              <Eye class="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#0D47A1] dark:text-blue-400 shrink-0" />
    ...
    >
      <div
        ...
        aria-modal="true"
      >
        <div class="relative w-full max-w-2xl bg-white dark:bg-slate-800 rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-200/80 dark:border-slate-700 overflow-hidden my-4 sm:my-6 max-h-[92vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
          <!-- Modal Hero Image Header -->
          <div class="relative h-44 sm:h-64 w-full shrink-0 overflow-hidden bg-slate-100 dark:bg-slate-700">
            <LazyImage
            ...
            <div
              ...
              class="w-full h-full flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-slate-700 dark:to-slate-800 text-blue-600"
            >
              <Wrench class="w-12 h-12 sm:w-16 sm:h-16 opacity-50" />
            </div>
            ...
            <button
              ...
              class="absolute top-3 right-3 w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-slate-900/80 hover:bg-slate-900 text-white flex items-center justify-center transition-all cursor-pointer backdrop-blur-md shadow-md hover:scale-105"
              aria-label="Close"
            >
            ...
            <div class="absolute top-3 left-3 flex items-center gap-1.5">
              <span class="px-2 sm:px-3 py-0.5 sm:py-1 rounded-lg sm:rounded-xl bg-blue-600/90 text-white text-[10px] sm:text-xs font-black backdrop-blur-md flex items-center gap-1 sm:gap-1.5 shadow-sm">
                <Wrench class="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                <span>{{ selectedService.category }}</span>
              </span>
              <span class="px-2 sm:px-3 py-0.5 sm:py-1 rounded-lg sm:rounded-xl bg-emerald-600/90 text-white text-[10px] sm:text-xs font-black backdrop-blur-md flex items-center gap-1 sm:gap-1.5 shadow-sm">
            ...
            <div class="absolute bottom-3 sm:bottom-4 left-3 sm:left-4 right-3 sm:right-4 text-white">
              <div class="flex items-center gap-1 sm:gap-1.5 text-[11px] sm:text-xs text-blue-200 font-bold mb-0.5 sm:mb-1 truncate">
                <span class="truncate">{{ selectedService.provider }}</span>
                <ShieldCheck class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-400 shrink-0" />
                <span class="text-white/60">•</span>
                <span class="flex items-center gap-1 truncate">
                  <MapPin class="w-3 h-3 sm:w-3.5 sm:h-3.5 text-red-400 shrink-0" />
                  <span class="truncate">{{ selectedService.location }}</span>
                </span>
              </div>
              <h2 class="text-base sm:text-2xl font-black leading-snug drop-shadow-sm text-white line-clamp-2">
          ...
          <div class="p-3 sm:p-5 bg-slate-50 dark:bg-slate-700/40 border-t border-slate-200/80 dark:border-slate-700 flex flex-wrap sm:flex-nowrap items-center gap-2">
            ...
            <button
              type="button"
              @click="closeServiceDetail"
              class="py-2.5 sm:py-3 px-3 sm:px-4 rounded-xl sm:rounded-2xl border border-slate-300 dark:border-slate-600 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 font-bold text-xs sm:text-sm transition-all cursor-pointer shrink-0"
            >
